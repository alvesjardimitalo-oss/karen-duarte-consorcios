...
export async function salvarProdutoAction(formData:FormData){
 ...
 const sku=String(formData.get('sku')??'').trim(),name=String(formData.get('name')??'').trim(),brand=String(formData.get('brand')??'').trim();
 if(!sku||!name||!brand)throw new Error('Código, produto e marca são obrigatórios.');
 const payload={sku,name,brand,category:String(formData.get('category')??'').trim()||null,description:String(formData.get('description')??'').trim()||null,image_url:String(formData.get('image_url')??'').trim()||null,active:true,updated_at:new Date().toISOString()};
