...
export async function criarConsorcioAction(formData: FormData) {
  ...
  const {error}=await supabase.rpc('create_consortium',{p_name:name,p_participant_limit:participantLimit,p_installment_amount:installmentAmount,p_duration_months:durationMonths,p_due_day:dueRule==='FIXED_DAY'?dueDay:1,p_draw_day:drawDay,p_starts_on:startsOn,p_due_rule:dueRule,p_due_business_day:dueRule==='BUSINESS_DAY'?dueBusinessDay:null,p_credit_amount:null});
  if(error)throw new Error(error.message); revalidatePath('/consorcios'); revalidatePath('/'); redirect('/consorcios');
}
