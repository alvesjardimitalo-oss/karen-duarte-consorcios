...
import { ArrowLeft, BellRing, Camera, UserRound, Users } from 'lucide-react';
...
export default async function ClientesPage() {
  ...
  const { data: accessRows } = isAdmin ? await supabase.from('client_access_requests').select('id,client_id,phone,status,request_type,requested_at,activation_expires_at,activation_attempts').in('status',['PENDENTE','APROVADA','PROCESSANDO']).order('requested_at',{ascending:true}) : { data: [] as any[] };
  const accessClientIds=[...new Set((accessRows??[]).map((r:any)=>r.client_id).filter(Boolean))];
  const { data: accessClients } = isAdmin && accessClientIds.length ? await supabase.from('clients').select('id,name').in('id',accessClientIds) : { data: [] as any[] };
  const accessClientNames=new Map((accessClients??[]).map((c:any)=>[c.id,c.name]));
  const accessRequests=(accessRows??[]).map((r:any)=>({...r,clients:{name:accessClientNames.get(r.client_id)??'Cliente'}}));
  ...
  return <><StaffNav profile={profile} active="clientes"/><main className="clients-page with-app-nav"><div className="clients-wrap">
    ...
    {isAdmin && <section id="solicitacoes-acesso" className="access-admin-panel">
      <div className="access-admin-title"><div className="access-admin-icon"><BellRing size={22}/></div><div><p className="eyebrow">SOLICITAÇÕES DE ACESSO</p><h2>{(accessRequests??[]).length ? `${(accessRequests??[]).length} cliente${(accessRequests??[]).length===1?'':'s'} aguardando liberação` : 'Nenhuma solicitação pendente'}</h2><p className="muted">Aprove aqui o primeiro acesso do cliente. Após aprovar, o código de ativação será exibido para envio.</p></div></div>
      <SolicitacoesAcesso items={(accessRequests??[]) as any}/>
    </section>}    <details className="client-form-card new-client-details"><summary><span><b>+ Novo cliente</b><small>Cadastrar uma nova pessoa</small></span></summary><div className="form-heading"><div><p className="eyebrow">NOVO CADASTRO</p><h2>Novo cliente</h2></div><span className="required-note">* somente nome e telefone são obrigatórios</span></div><form action={criarClienteAction} className="client-form"><label className="photo-field"><div className="avatar-placeholder"><UserRound size={34}/><span className="camera-badge"><Camera size={14}/></span></div><div><strong>Foto de perfil</strong><small>JPG, PNG ou WebP · até 5 MB</small><AvatarFileInput/></div></label><div className="form-grid"><label><span>Nome *</span><input name="nome" required minLength={2} placeholder="Nome do cliente" /></label><label><span>Telefone *</span><input name="telefone" required inputMode="tel" placeholder="(33) 99999-9999" /></label><label><span>CPF</span><input name="cpf" inputMode="numeric" placeholder="Opcional" /></label><label><span>Tags</span><input name="tags" placeholder="VIP, indicação, bairro..." /></label><label className="full"><span>Endereço</span><input name="endereco" placeholder="Opcional" /></label></div><button className="primary save-client" type="submit">Cadastrar cliente</button></form></details>
