...
export default async function SorteiosPage() {
  ...
  const [{ data: consortia }, { data: draws }, { data: members }] = await Promise.all([
    ...
    supabase.from('draws').select('id,consortium_id,draw_number,scheduled_for,completed_at,winner_member_id,consortia(name)').order('scheduled_for', { ascending: true }),
    supabase.from('consortium_members').select('id,consortium_id,clients(name),contemplations(id)').order('joined_at')
  ]);
      ...
      <section className="client-list-card">
        ...
        <div className="draw-list-modern">{ds.length===0?<div className="empty-clients"><Trophy size={34}/><strong>Nenhum sorteio agendado</strong><span>Use o formulário acima para criar o primeiro.</span></div>:ds.map(d=>{
          ...
          return <article className={'draw-card-modern '+(d.completed_at?'is-completed':'')} key={d.id}>
            ...
            <div className="draw-card-info"><span className={'draw-status '+(d.completed_at?'completed':'scheduled')}>{d.completed_at?<><CheckCircle2 size={14}/>Concluído</>:<><Clock3 size={14}/>Agendado</>}</span><h3>{d.consortia?.name} · Sorteio {d.draw_number}</h3><p><Users size={15}/>{eligible.length} participante{eligible.length===1?'':'s'} elegível{eligible.length===1?'':'eis'}</p>{d.completed_at&&<strong className="winner-name"><Trophy size={15}/> Contemplado: {winner?.clients?.name??'—'}</strong>}</div>
